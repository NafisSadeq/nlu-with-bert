...
def select_backend(embedding_model,
                   language: str = None, **kwargs) -> BaseEmbedder:
    ...
    if language:
        if language.lower() in ["English", "english", "en"]:
            return SentenceTransformerBackend("all-MiniLM-L6-v2", **kwargs)
        elif language.lower() in languages or language == "multilingual":
            return SentenceTransformerBackend("paraphrase-multilingual-MiniLM-L12-v2", **kwargs)
        else:

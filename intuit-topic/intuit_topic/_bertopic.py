...
import yaml
from intuit_topic import plotting
# BERTopic
from intuit_topic._ctfidf import ClassTFIDF
from intuit_topic._mmr import mmr
from intuit_topic._utils import (MyLogger, check_documents_type,
                                 check_embeddings_shape, check_is_fitted)
from intuit_topic.backend._utils import select_backend
...
class BERTopic:
    ...
    def __init__(self,
        ...
        print(
            'Created {} object - call .fit() or .fit_transform() to train model!'
    ...
    def fit_transform(
        ...
        if embeddings is None:
            ...
            print('{} ({} total) - Transformed documents to embeddings'.format(
                datetime.now() - start_embed,
        ...
        print('{} ({} total) - Reduced dimensionality with UMAP'.format(
            datetime.now() - start_umap,
            datetime.now() - start_all))

        # Cluster UMAP embeddings with HDBSCAN
        start_hdbscan = datetime.now()
        documents, probabilities = self._cluster_embeddings(
            umap_embeddings, documents)
        print('{} ({} total) - Clustered UMAP embeddings with HDBSCAN'.format(
            datetime.now() - start_hdbscan,
            datetime.now() - start_all))
        ...
        # Reduce topics
        if self.nr_topics:
            documents, _ = self._reduce_topics(documents)
    ...
    def transform(
        ...
        if self.calculate_probabilities:
            probabilities = hdbscan.membership_vector(self.hdbscan_model,
                                                      umap_embeddings)
    ...
    def topics_over_time(self,
        ...
        global_c_tf_idf = normalize(self.c_tf_idf,
                                    axis=1,
                                    norm='l1',
                                    copy=False)
        ...
        if nr_bins:
            ...
            documents["Timestamps"] = documents.apply(
                lambda row: row.Bins.left, 1)
        ...
        for index, timestamp in tqdm(enumerate(timestamps),
            ...
            documents_per_topic = selection.groupby(['Topic'],
                                                    as_index=False).agg({
                                                        'Document':
                                                        ' '.join,
                                                        "Timestamps":
            ...
            if evolution_tuning and index != 0:
                ...
                overlapping_topics = sorted(
                    list(
                        set(previous_topics).intersection(
                            set(current_topics))))
            ...
            words_per_topic = self._extract_words_per_topic(
                words, c_tf_idf, labels)
            topic_frequency = pd.Series(
                documents_per_topic.Timestamps.values,
                index=documents_per_topic.Topic).to_dict()

            # Fill dataframe with results
            topics_at_timestamp = [
                (topic, ", ".join([words[0] for words in values][:5]),
                 topic_frequency[topic], timestamp)
                for topic, values in words_per_topic.items()
            ]
            topics_over_time.extend(topics_at_timestamp)

            if evolution_tuning:
                previous_topics = sorted(list(
                    documents_per_topic.Topic.values))
    ...
    def topics_per_class(self,
        ...
        # For each unique timestamp, create topic representations
        topics_per_class = []
        for index, class_ in tqdm(enumerate(set(classes)),
                                  disable=not self.verbose):
            ...
            if global_tuning:
                c_tf_idf = normalize(c_tf_idf, axis=1, norm='l1', copy=False)
                c_tf_idf = (global_c_tf_idf[documents_per_topic.Topic.values +
                                            1] + c_tf_idf) / 2.0
            ...
            words_per_topic = self._extract_words_per_topic(
                words, c_tf_idf, labels)
            topic_frequency = pd.Series(
                documents_per_topic.Class.values,
                index=documents_per_topic.Topic).to_dict()

            # Fill dataframe with results
            topics_at_class = [
                (topic, ", ".join([words[0] for words in values][:5]),
                 topic_frequency[topic], class_)
                for topic, values in words_per_topic.items()
            ]
    ...
    def find_topics(self,
        ...
        sims = cosine_similarity(search_embedding.reshape(1, -1),
                                 self.topic_embeddings).flatten()
    ...
    def get_topic(self,
                  topic: int) -> Union[Mapping[str, Tuple[str, float]], bool]:
    ...
    def get_topic_info(self, topic: int = None) -> pd.DataFrame:
        ...
        info = pd.DataFrame(self.topic_sizes.items(),
                            columns=['Topic',
    ...
    def get_topic_freq(self, topic: int = None) -> Union[pd.DataFrame, int]:
        ...
        if isinstance(topic, int):
            ...
            return pd.DataFrame(self.topic_sizes.items(),
                                columns=['Topic',
                                         'Count']).sort_values("Count",
                                                               ascending=False)
    ...
    def reduce_topics(self,
        ...
        document, mapped_topics = self._reduce_topics(documents)
        self.merged_topics = None
        ...
        return new_topics, new_probabilities, mapped_topics
    ...
    def _cluster_embeddings(
            ...
            documents: pd.DataFrame) -> Tuple[pd.DataFrame, np.ndarray]:
        """ Cluster UMAP embeddings with HDBSCAN
        ...
        if self.calculate_probabilities:
            probabilities = hdbscan.all_points_membership_vectors(
                self.hdbscan_model)
    ...
    def _guided_topic_modeling(
            self, embeddings: np.ndarray) -> Tuple[List[int], np.array]:
        ...
        seed_topic_list = [
            " ".join(seed_topic) for seed_topic in self.seed_topic_list
        ]
        seed_topic_embeddings = self._extract_embeddings(seed_topic_list,
                                                         verbose=self.verbose)
        ...
        for seed_topic in range(len(seed_topic_list)):
            indices = [
                index for index, topic in enumerate(y) if topic == seed_topic
            ]
            embeddings[indices] = np.average(
                [embeddings[indices], seed_topic_embeddings[seed_topic]],
                weights=[3, 1])
        return y, embeddings
        ...
        for topic in documents['Topic'].unique():
            if topic != -1:
                leaves = hdbscan.plots._recurse_leaf_dfs(
                    cluster_tree, clusters[topic])

                result = np.array([])
                for leaf in leaves:
                    max_lambda = raw_tree['lambda_val'][raw_tree['parent'] ==
                                                        leaf].max()
        ...
        if self.embedding_model is not None:
            ...
            for i, topic in enumerate(topic_list):
                ...
                if sum(word_importance) == 0:
                    word_importance = [
                        1 for _ in range(len(self.get_topic(topic)))
                    ]
    ...
    def _c_tf_idf(self,
                  ...
                  fit: bool = True) -> Tuple[csr_matrix, List[str]]:
        ...
        if self.seed_topic_list:
            seed_topic_list = [
                seed for seeds in self.seed_topic_list for seed in seeds
            ]
            multiplier = np.array(
    ...
    def _update_topic_size(self, documents: pd.DataFrame):
        ...
        """
        sizes = documents.groupby(['Topic']).count().sort_values(
    ...
    def _extract_words_per_topic(
            ...
            labels: List[int] = None) -> Mapping[str, List[Tuple[str, float]]]:
        ...
        topics = {
            label: [(words[word_index], score) if word_index and score > 0 else
                    ("", 0.00001) for word_index, score in zip(
    ...
    def _reduce_topics(self, documents: pd.DataFrame) -> pd.DataFrame:
        ...
        if isinstance(self.nr_topics, int):
            if self.nr_topics < initial_nr_topics:
                documents, mapped_topics = self._reduce_to_n_topics(documents)
        elif isinstance(self.nr_topics, str):
            documents, mapped_topics = self._auto_reduce_topics(documents)
        else:
        ...
        return documents, mapped_topics
    ...
    def _reduce_to_n_topics(self, documents: pd.DataFrame) -> pd.DataFrame:
        ...
        print('{} [_reduce_to_n_topics] created {} similarities matrix'.format(
            datetime.now() - start, similarities.shape))
        ...
        while len(self.get_topic_freq()) > self.nr_topics + 1:
            ...
            topic_to_merge_into = np.argmax(
                similarities[topic_to_merge + 1]) - 1
            similarities[:, topic_to_merge + 1] = -1
            self.merged_topics.append(topic_to_merge)

            # Update Topic labels
            documents.loc[documents.Topic == topic_to_merge,
                          "Topic"] = topic_to_merge_into
            mapped_topics[topic_to_merge] = topic_to_merge_into
            self._update_topic_size(documents)
            sys.stdout.write('.')
        print('{} [_reduce_to_n_topics] Created topic map'.format(
            datetime.now() - start))

        # Map topics
        mapped_topics = {
            from_topic: to_topic
            for from_topic, to_topic in tqdm(
                zip(topics, documents.Topic.tolist()), total=len(topics))
        }
        self.topic_mapper.add_mappings(mapped_topics)
        print('{} [_reduce_to_n_topics] Mapped topics'.format(datetime.now() -
                                                              start))
        ...
        self._extract_topics(documents)
        self._update_topic_size(documents)
        print('{} [_reduce_to_n_topics] Updated representations'.format(
            datetime.now() - start))
        return documents, mapped_topics
    ...
    def _auto_reduce_topics(self, documents: pd.DataFrame) -> pd.DataFrame:
        ...
        predictions = hdbscan.HDBSCAN(min_cluster_size=2,
                                      metric='euclidean',
                                      cluster_selection_method='eom',
                                      prediction_data=True).fit_predict(
                                          norm_data[1:])

        # Map similar topics
        mapped_topics = {
            unique_topics[index]: prediction + max_topic
            for index, prediction in enumerate(predictions) if prediction != -1
        }
        documents.Topic = documents.Topic.map(mapped_topics).fillna(
            documents.Topic).astype(int)
        mapped_topics = {
            from_topic: to_topic
    ...
    def _sort_mappings_by_frequency(self,
                                    documents: pd.DataFrame) -> pd.DataFrame:
        ...
        df = pd.DataFrame(self.topic_sizes.items(),
                          columns=["Old_Topic",
        ...
        # Map documents
        documents.Topic = documents.Topic.map(sorted_topics).fillna(
            documents.Topic).astype(int)
        self._update_topic_size(documents)
        return documents
    ...
    def _top_n_idx_sparse(matrix: csr_matrix, n: int) -> np.ndarray:
        ...
        for le, ri in zip(matrix.indptr[:-1], matrix.indptr[1:]):
            ...
            values = matrix.indices[
                le +
                np.argpartition(matrix.data[le:ri], -n_row_pick)[-n_row_pick:]]
            values = [
    ...
    @staticmethod
    def _top_n_values_sparse(matrix: csr_matrix,

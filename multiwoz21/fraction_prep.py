...
def load_slot_freq():
    slotFreq={}
    with open('slot_freq.json','r') as file:
        slotFreq=json.loads(file.read())
    return slotFreq

def utt_score(item,slotFreq):
    INF=1000000
    slot_score_list=[INF]

    for slot_triple in item[3]:

        slot_name=slot_triple[0]+'+'+slot_triple[1]
        slot_score=INF
        if(slot_name in slotFreq):
            slot_score=slotFreq[slot_name]
        slot_score_list.append(slot_score)

    return min(slot_score_list)

def filter_train(train_list,frac_len):

    slotFreq=load_slot_freq()
    filtered_train=[]

    for item in train_list:
        utt_sc=utt_score(item,slotFreq)
        filtered_train.append((utt_sc,item))

    new_train=filtered_train[:frac_len-frac_len//10]
    filtered_train.sort()
    new_train=new_train+filtered_train[:frac_len//10]
    new_train=[item[1] for item in new_train]

    return new_train

def preprocess(mode,frac):
    ...
    for key in data_key:
        ...
        data_len=len(processed_data[key])

        if(key=='train'):

            frac_len=int(data_len*float(frac))
            processed_data[key]=filter_train(processed_data[key],frac_len)
            print('fraction {}, size {}'.format(key,frac_len))
        
        print('loaded {}, size {}'.format(key,data_len))

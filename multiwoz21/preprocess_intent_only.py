            ...
            for is_sys, turn in enumerate(sess['log']):
                ...
                for intent in intents:
                    if(intent in intent_mapping):
                        mapped_intents.append(intent_mapping[intent])
                    else:
                        intent_mapping[intent]="intent."+str(intent_count).zfill(3)
                        mapped_intents.append(intent_mapping[intent])
                        intent_count+=1

                processed_data[key].append([tokens, tags, mapped_intents,[],[]])
                all_da += [da for da in turn['dialog_act']]
                all_intent += mapped_intents
                all_slots +=slots
                all_tag += tags
